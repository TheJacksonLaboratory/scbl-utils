...
def tracking_spec(spec: dict[str, dict[str, str] | str], required_columns : set = REQUIRED_TRACKINGSHEET_SPEC_COLUMNS, index_col: str = TRACKING_DF_INDEX_COL, required_lib_types: set = LIB_TYPES):
    sheets = spec['sheets']
    spec_columns = {col for sheet_dict in sheets.values() for col in sheet_dict['columns'].values()}
    missing_columns = '\n'.join(required_columns - spec_columns)
    if missing_columns:
        sheet_id = spec['id']
        rprint(f'The following columns are missing from [green]trackingsheet-spec.yml[/]:', f'[bright_cyan]{missing_columns}[/]', f'Please map the columns in https://docs.google.com/spreadsheets/d/{sheet_id} to these missing columns and place mapping in [bright_cyan]trackingsheet-spec.yml[/].', sep='\n\n')
        raise Abort()
    ...
    if missing_lib_types:
        rprint(f'The following library types are not present in [bright_cyan]trackingsheet-spec.yml[/], but are required:', f'[bright_cyan]{missing_lib_types}[/]', sep='\n\n')
        raise Abort()
...
def metrics_spec(spec: dict[str, str | dict[str, str] | int], required_columns: set = REQUIRED_METRICSSHEET_SPEC_KEYS):
    ...
    if missing_columns:
        rprint(f'The following columns are missing from [green]metricssheet-spec.yml[/]:', f'[bright_cyan]{missing_columns}[/]', f'Please map the columns in a given delivery metrics sheet to these missing columns and place mapping in [bright_cyan]metricssheet-spec.yml[/].', sep='\n\n')
        raise Abort()
    return
